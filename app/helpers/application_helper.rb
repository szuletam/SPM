...
module ApplicationHelper
  ...
  def link_to_issue(issue, options={})
    ...
    s << ": #{issue.topic.first.detail}" if (issue.is_task? || issue.is_desition?) && issue.topic.first && !issue.topic.first.detail.blank?
  ...
  def render_project_nested_lists(projects, &block)
    ...
    if projects.any?
      ...
      projects.sort_by(&:lft).each do |project|
        ...
        classes += " inherit" if project.inherit_members?
        classes += " parent_#{project.parent_id}" if project.parent_id
  ...
  def project_tree_node project, link = true
    return '' unless project
    classes = ''
    classes += 'project-visible' if project.visible?
    html = "<li id='node_#{project.id}' data-visible='#{project.visible?}' class='#{classes}'>"
    html += project.name
    if project.children.active.any?
      project.children.active.each do |children|
        html += '<ul>'
        html += project_tree_node children
        html += '</ul>'
      end
    end
    html += '</li>'
  ...
  def javascript_heads
    tags = javascript_include_tag('jquery-1.11.1-ui-1.11.0-ujs-3.1.4', 'application.min', 'responsive')
