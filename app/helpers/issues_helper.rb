...
module IssuesHelper
  ...
  #for_table = desition or issue
  def render_strategic_projects(for_table, random, row, selected = nil)
    projects = Project.visible.active.where(:project_type_id => Setting.default_project_for_strategy)
    project_selected = nil
    if selected
      project_selected = Project.find(selected)
      projects += [project_selected] unless projects.map{|p| p.id.to_s}.include?(selected.to_s)
    end
    options = ("<option value=''>#{ l(:label_choose_a_project) }</option>" +
  ...
  def update_issue_form_path(project, issue, issue_with_project = true)
    options = {:format => 'js'}
    if issue.new_record?
