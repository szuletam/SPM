...
module PrincipalMembershipsHelper
  def render_principal_memberships(principal)
    projects_tree = Project.project_tree(principal.memberships.map{|m| m.project}){|p| p}
    memberships = principal.memberships.sort_by do |m|
      projects_tree.index(m.project)
    end

    render :partial => 'principal_memberships/index', :locals => {principal: principal, memberships: memberships}
  end
