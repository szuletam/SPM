...
ysy.pro.ggrm = {
  _name: "GGRM",
  patch: function () {
    if (!ysy.pro.resource || !ysy.pro.resource.project_canvas_renderer) return;
    ysy.proManager.register("close",this.close);
    var ggrmClass = ysy.pro.ggrm;
    $(".gantt-menu-button.task-control").after(
        ' <a class="gantt-menu-button ggrm button button-2 icon icon-stats" id="button_ggrm" href="javascript:void(0)">'
        + ysy.settings.labels.resources.label_resources
        + '</a>');
