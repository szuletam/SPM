...
function dhtmlxDnD(obj, config) {
    ...
    dhtmlxEvent(obj, "mousedown", dhtmlx.bind(function(e) {
        ...
        this.dragStart(obj, e); // aqui comienza el drag
    }, this));
...
gantt._render_grid_item = function (item) {
  ////cconssole.log("_render_grid_item"); // adn
  if (!gantt._is_grid_visible())
    return null;
  if(gantt._get_safe_type(item.type) !== "task"){
...
gantt._scale_helpers = {
  ...
  eachColumn : function(unit, step, callback){
    ...
    var curr = gantt.date.Date(start);
    if(+curr >= +end){
  ...
  limitVisibleRange : function(cfg){
    ...
    if(+dates[0] < +gantt._min_date && left != right){
      ...
      dates[0] = gantt.date.Date(gantt._min_date);
    }
...
gantt._tasks_dnd = {
  ...
  _resize : function(ev, shift, drag){
    ...
    if(drag.left){
      ...
      if(!gantt.ascStop(ev,start_date)){return;}
      ev.start_date=start_date;
      if(!ev.start_date){
        ev.start_date = gantt.date.Date(gantt.getState().min_date);
      }
    }else{
      ev.end_date = moment(gantt.dateFromPos(coords_x.end + shift.x)).subtract(1,"days");
      if(!ev.end_date){
  ...
  _move : function(ev, shift, drag){
    // TODO konce tasku
		if(ev.id.toString().indexOf('p') > -1 || ev.id.toString().indexOf('m') > -1) // No move users ADN
		{
	    return;
		}
    var coords_x = this._drag_task_coords(ev, drag);
    var new_start = moment(gantt.dateFromPos(coords_x.start + shift.x)),
    new_end = moment(gantt.dateFromPos(coords_x.end + shift.x)).subtract(1,"days");
    ////cconssole.log("Desde ", coords_x.start + shift.x, " es " , moment(gantt.dateFromPos(coords_x.start + shift.x))._d, " Hasta ", coords_x.end + shift.x," es ", moment(gantt.dateFromPos(coords_x.end + shift.x)).subtract(1,"days")._d);
		new_end._isEndDate=true; // HOSEK < A V
    if(!new_start.isValid()||!new_end.isValid()){
      debugger;
    }
    gantt.ascStop(ev,new_start,new_end);
    ...
    if(!new_start){
      ...
      ev.end_date = new_end;
			if(ev.type == 'milestone'){
				ev.real_end_date = new_end.startOf('day'); // arreglar
				ev.real_end_date = gantt.dateFromPos(gantt.posFromDate(new_end));
			}
    }
		////cconssole.log("_move");
    gantt.moveDesc(ev, ev.end_date);
    gantt.moveChildren(ev,ev.start_date.diff(old_start,"seconds"));
    ////cconssole.log(new_start.toString()+" "+new_end.toString());
  },
  ...
  _update_on_move : function(){
    ...
    if (drag&&drag.mode){
      ...
      if(this._handlers[drag.mode]){
        ...
        dhtmlx.mixin(ev, copy, true); // mueve adn
        ...
        gantt.refreshTask(drag.id); // genera transicion
      }
    }
  },
  ...
  _fix_working_times:function(task, drag){
    ////cconssole.log("_fix_working_times REWRITED");
    gantt._working_time_helper.round_date(task.start_date);
    if(drag.mode == gantt.config.drag_mode.resize){
        gantt._working_time_helper.round_date(task.end_date);
...
gantt._render_link_element = function(link){
  ...
  // HOSEK
  if(link.delay>0){
    var delaypos={x:(dots[dots.length-2].x+dots[dots.length-1].x)/2-5,y:dots[dots.length-2].y};
...
////cconssole.log("init_links_dnd");
  ...
  dnd.attachEvent("onDragEnd", dhtmlx.bind(function() {
    ...
    if(drag.from && drag.to && drag.from != drag.to){
      var type = gantt.f(drag.from_start, drag.to_start);
...
gantt._task_default_render = function(task){
  ...
  if(task.real_end_date != undefined && 'milestone' == task.type){
	  task.end_date = task.real_end_date;
  }
  var pos = this._get_task_pos(task);
	////cconssole.log("aca", task.id, pos);
  var cfg = this.config;
  ...
  ////cconssole.log(type, cfg.show_progress, (controls.show_progress || controls.progress));
  ////cconssole.log(this.config);
  if(cfg.show_progress && (controls.show_progress || controls.progress)){
    var progress = this._render_task_progress(task,div, width);
		////cconssole.log(progress, width); adn
...
gantt.dateFromPos2 = function(x){
  ...
  if(!scale.trace_x.length){return 0;}
  var units={
    day:24*60*60*1000,
    week:7*24*60*60*1000,
    month:30*24*60*60*1000
  };
  var date = gantt.date.Date(scale.trace_x[0].valueOf() + Math.round(x/scale.col_width*units[scale.unit]));
  return date;
};
...
gantt._get_task_coord = function(task, to_start, x_correction){
  ...
  //var isMilestone = (this._get_safe_type(task.type) == this.config.types.milestone); adn
  var isMilestone = false; //adn
  // es aqui
  var date = null;
  ...
  if(to_start){
    ...
    date = (task.end_date || this.calculateEndDate(this._default_task_date(task)));
		if(task.id.toString().indexOf('m') > -1){
			try{
			  //console.log(task.end_date._d, 'm');
			}
			catch(exc){
				//console.log(task.id);
			}
		}
		else if(task.id.toString().indexOf('p') > -1){
			try{
			  ////cconssole.log(task.end_date._d, 'p');
			}
			catch(exc){}
		}
  }
  ...
  var x = this.posFromDate(date),
	...
	////cconssole.log(x, y, task.id);
  ////cconssole.log(" x , y, id  = ", [x, y, task.id]); // adn
  if(isMilestone){
    if(to_start){
      x -= x_correction;
    }else{
...
gantt._get_mouse_pos = function(ev){
  ...
  //var offset = ysy.data.limits.offset;
  ////var offset = $(gantt.$task_data).children().first().offset();
  //var offpos={x:pos.x-offset.left,y:pos.y-offset.top};
  //return offpos;
  ...
  //ysy.log.debug("POS=["+pos.x+","+pos.y+"] OFFPOS=["+offpos.x+","+offpos.y+"]");
  return pos;
};
...
gantt.addTaskFaster = function (item, parent) {
	////cconssole.log("addTaskFaster", item.open);
...
// downward calculation of project duration
gantt.resetMilestoneDates = function(task){
	var dates = this.getSubtaskDates(task.id);
	ysy.log.debug("resetProjectDates to start="+dates.start_date+" end="+dates.end_date,"parent");
	////cconssole.log("resetProjectDates to start="+dates.start_date+" end="+dates.end_date,"parent");
	this._assign_milestone_dates(task, dates.start_date, dates.end_date);
};
...
gantt.getSubtaskDates = function(task_id){
  ...
  this.eachTask(function(child){
    ...
    if(dates.start_date){
      if(!min || min > dates.start_date.valueOf()){
        min=dates.start_date.valueOf();
      }
    }
    if(dates.end_date){
      if(!max || max < dates.end_date.valueOf()){
        max=dates.end_date.valueOf();
      }
    }
...
gantt._assign_milestone_dates = function(task, from, to){
  task.start_date = gantt.date.Date(from);
	...
	//console.log(task.start_date._d, task.end_date._d, this.config.duration_step/*, from._d, to._d*/); // anderson
};
...
gantt._update_parents = function(taskId, silent){
  ...
  var pid = this.getParent(task);
	...
	if(task.id.toString().indexOf('m') > -1) {
		gantt.resetMilestoneDates(task);
		
		//console.log(task.end_date._d);
    //if(!silent){
...
gantt.scrollTo = function(left, top){
  ysy.log.debug("ScrollTo("+left+","+top+")","scroll");
  if (left*1 == left){
...
gantt._init_templates = function(){
  ...
  dhtmlx.mixin(this.templates,{
    ...
    grid_folder:function(item) {
      ////cconssole.log("grid_folder");
      if(item.$open||item.type===gantt.config.types.milestone||item.type===gantt.config.types.project){  //  HOSEK
    ...
    grid_file:function(item) {
      if(item.type!==gantt.config.types.milestone && item.type!==gantt.config.types.project)
        return "<div class='gantt_tree_icon'><div class='gantt_drag_handle gantt_alldirarrow'></div></div>";  // HOSEK
