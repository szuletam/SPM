...
class Issue < ActiveRecord::Base
  ...
  acts_as_watchable
  acts_as_searchable :columns => ['subject', "#{table_name}.description", "#{table_name}.spmid"],
                     :preload => [:project, :status, :tracker],
                     :scope => lambda {|options| options[:open_issues] ? self.open : self.all}

  acts_as_event :title => Proc.new {|o| "#{o.tracker.name} ##{o.spmid} (#{o.status}): #{o.subject}"},
  ...
  def self.visible_condition(user, options={})
    Project.allowed_to_condition(user, :view_issues, options) do |role, user|
      sql = if user.id && user.logged?
        ...
        when 'own'
          user_ids = [user.id] + user.groups.map(&:id).compact
          issues_my_direction = user.director? ? "OR (#{table_name}.direction_id = '#{user.direction_id}')" : ""
          my_projects = Member.get_owner_projects(user)
          owner_projects = []
