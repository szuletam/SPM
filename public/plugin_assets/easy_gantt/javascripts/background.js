  ...
  return {
    ...
    _render_bg_canvas: function (svg, items, limits) {
      ...
      if (gantt.config.scale_unit === "day") {
        var weekendGroup = svg.group().attr('fill', colors.weekend);
      ...
      if (ysy.settings.resource.open) {
        //  -- ASSIGNEE BACKGROUND --
        var assigneeGroup = svg.group().attr('fill', colors.assignee);
        for (i = limits.fromY; i < limits.toY; i++) {
        ...
        //  -- DARK LIMITS --
        var darkLimitGroup = svg.group().attr('fill', colors.darkLimits);
