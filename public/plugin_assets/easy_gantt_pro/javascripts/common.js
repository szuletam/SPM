    ...
    ysy.view.columnBuilders.name = function (obj) {
      ...
      if (obj.type === "project") {
        ...
        return "<a class='gantt-grid-row-project' href='" + path + id + "' title='" + obj.text + "' target='_blank'>" + obj.text + "</a>"
            + (nonLoadedIssues ? "<a class='gantt-grid-project-issues-expand' href='javascript:void(0)' title='Load " + nonLoadedIssues + " issues' onclick='ysy.data.loader.loadProjectIssues(" + id + ")'>" + nonLoadedIssues + "</a>" : "");
      }
    };
